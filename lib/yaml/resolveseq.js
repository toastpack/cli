...
class Collection extends Node {
  ...
  toString(
    ...
    const inFlow =
      this.type === _Type.FLOW_MAP ||
      this.type === _Type.FLOW_SEQ ||
      ctx.inFlow;
      ...
      (!key ||
        ...
        (key instanceof Node
          ? key instanceof Collection ||
            key.type === _Type.BLOCK_FOLDED ||
            key.type === _Type.BLOCK_LITERAL
          : typeof key === 'object'));
...
function blockString({ comment, type, value }, ctx, onComment, onChompKeep) {
  ...
  const literal =
    type === _Type.BLOCK_FOLDED
      ? false
      : type === _Type.BLOCK_LITERAL
      ? true
...
function stringifyString(item, ctx, onComment, onChompKeep) {
  ...
  const _stringify = (_type) => {
    switch (_type) {
      case _Type.BLOCK_FOLDED:
      case _Type.BLOCK_LITERAL:
        return blockString(item, ctx, onComment, onChompKeep);

      case _Type.QUOTE_DOUBLE:
        return doubleQuotedString(value, ctx);

      case _Type.QUOTE_SINGLE:
  ...
  if (
    ...
    (implicitKey || inFlow) &&
    (type === _Type.BLOCK_FOLDED || type === _Type.BLOCK_LITERAL)
  ) {
...
function checkFlowCollectionEnd(errors, cst) {
  ...
  for (let i = cst.items.length - 1; i >= 0; --i) {
    ...
    if (!item || item.type !== _Type.COMMENT) {
      lastItem = item;
      break;
    }
  }
...
function resolveNode(doc, node) {
  ...
  if (node.type === _Type.ALIAS && (hasAnchor || hasTag)) {
...
function resolveMap(doc, cst) {
  if (cst.type !== _Type.MAP && cst.type !== _Type.FLOW_MAP) {
    ...
    if (doc.schema.merge && iKey && iKey.value === MERGE_KEY) {
      ...
      sources.some((node) => {
        if (node instanceof Alias) {
          ...
          if (type === _Type.MAP || type === _Type.FLOW_MAP) return false;
      ...
      case _Type.MAP_VALUE:
        {
          ...
          if (!valueNode && item.props.length > 0) {
            ...
            valueNode = new PlainValue(_Type.PLAIN, []);
            valueNode.context = {
              parent: item,
...
function resolveFlowMapItems(doc, cst) {
  ...
  for (let i = 0; i < cst.items.length; ++i) {
    ...
    if (typeof item.char === 'string') {
      ...
      err.offset = offset;
      doc.errors.push(err);
    } else if (item.type === _Type.BLANK_LINE) {
      comments.push({
        afterKey: !!key,
        before: items.length,
      });
    } else if (item.type === _Type.COMMENT) {
...
function resolveSeq(doc, cst) {
  if (cst.type !== _Type.SEQ && cst.type !== _Type.FLOW_SEQ) {
