...
import { Document as _Document } from './documents.js';
...
function testEvents(src, options) {
  ...
  const docs = parse(src).map(cstDoc => new _Document(opt).parse(cstDoc));
...
const _testEvents = testEvents;
export { _testEvents as testEvents };
