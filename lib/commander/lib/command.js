...
Expecting one of '${allowedValues.join("', '")}'`);
  ...
  _executeSubCommand(subcommand, args) {
    ...
    function findFile(baseDir, baseName) {
      const localBin = resolve(baseDir, baseName);
      if (existsSync(localBin)) return localBin;

      if (sourceExt.includes(extname(baseName))) return undefined;

      const foundExt = sourceExt.find((ext) => existsSync(`${localBin}${ext}`));
    ...
    if (this._scriptPath) {
      ...
      try {
        resolvedScriptPath = realpathSync(this._scriptPath);
      } catch (err) {
        resolvedScriptPath = this._scriptPath;
      }
      executableDir = resolve(dirname(resolvedScriptPath), executableDir);
    }
    ...
    if (executableDir) {
      ...
      if (!localFile && !subcommand._executableFile && this._scriptPath) {
        const legacyName = basename(
          this._scriptPath,
          extname(this._scriptPath)
        );
    ...
    launchWithNode = sourceExt.includes(extname(executableFile));
    ...
    if (process.platform !== 'win32') {
      if (launchWithNode) {
        ...
        proc = spawn(executableFile, args, { stdio: 'inherit' });
      }
    } else {
      args.unshift(executableFile);
 ...
 - ${executableDirMessage}`;
  ...
  nameFromFilename(filename) {
    this._name = basename(filename, extname(filename));
